...
static bool validate_string(debug_report_data *report_data, const char *apiName, const ParameterName &stringName,
    ...
    if (result == VK_STRING_ERROR_NONE) {
        ...
        skip = log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                       INVALID_USAGE, "%s: string %s exceeds max length %d", apiName, stringName.get_name().c_str(),
                       MaxParamCheckerStringLength);
    } else if (result & VK_STRING_ERROR_BAD_DATA) {
        skip = log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                       INVALID_USAGE, "%s: string %s contains invalid characters or is badly formed", apiName,
...
bool pv_VkViewport(const layer_data *device_data, const VkViewport &viewport, const char *fn_name, const char *param_name,
    ...
    if (!(viewport.width > 0.0f)) {
        ...
        skip |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, object_type, object, VALIDATION_ERROR_15000dd6,
                        "%s: %s.width (=%f) exceeds VkPhysicalDeviceLimits::maxViewportDimensions[0] (=%" PRIu32 ").",
    ...
    if (!negative_height_enabled && !(viewport.height > 0.0f)) {
        ...
        skip |= log_msg(
            report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, object_type, object, VALIDATION_ERROR_15000dda,
    ...
    if (x_healthy && width_healthy) {
        ...
        if (!(right_bound <= device_data->device_limits.viewportBoundsRange[1])) {
            skip |= log_msg(
                report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, object_type, object, VALIDATION_ERROR_150009a0,
    ...
    if (y_healthy && height_healthy) {
        ...
        if (!(boundary <= device_data->device_limits.viewportBoundsRange[1])) {
            skip |= log_msg(
                report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, object_type, object, VALIDATION_ERROR_150009a2,
